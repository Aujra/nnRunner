...
-- create a subclass
function Class:extend(obj, class_name)
    local obj = obj or {}
    obj.__class__ = class_name and class_name or "class"
...
function Class:new(...)
    ...
    if obj.init then
        --print("calling init on: " .. tostring(obj.__class__))
        obj:init(...)
    end
    return obj
end
...
function class(attr, class_name)
    attr = attr and attr or {}
    attr.__class__ = class_name and class_name or "class"

    return Class:extend(attr)
end

function dump_docs(class)
end
